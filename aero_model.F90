...
!         if (lphase == 1) then
  ...
  subroutine aero_model_wetdep( state, dt, dlf, cam_out, ptend, pbuf)
    ...
    if (modeptr_coarse > 0) then  !kzm apply to the coarse mode if there is dust inside
    ...
    do m = 1, ntot_amode ! main loop over aerosol modes
       ...
       do lphase = 1, 2 ! loop over interstitial (1) and cloud-borne (2) forms
          ...
          if (lphase == 1) then ! interstial aerosol
             ...
             ! added by kzm
             else if ((m == modeptr_coardust1) .or. (m == modeptr_coardust2) .or. (m == modeptr_coardust3)) then
                f_act_conv = 0.4_r8 
             !-------------
             else
