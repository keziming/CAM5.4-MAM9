...
contains
  ...
  subroutine modal_aero_register
    ...
    character(len=*), parameter :: xname_numptr(ntot_amode)   = (/ 'num_a1  ', 'num_a2  ', 'num_a3  ', &
         'num_a4  ', 'num_a5  ', 'num_a6  ', 'num_a7  ', 'num_a8  ','num_a9  ' /)
    character(len=*), parameter ::     xname_numptrcw(ntot_amode) = (/ 'num_c1  ', 'num_c2  ', 'num_c3  ', &
         'num_c4  ', 'num_c5  ', 'num_c6  ', 'num_c7  ', 'num_c8  ','num_c9  ' /) !kzm
...
! added by kzm
     ...
     ! mode 8 (coarse dust) species
       xname_massptr(:nspec_amode(8),8)   = (/ 'dst_a8  ', 'so4_a8  ', 'nh4_a8  ' /)
       xname_massptrcw(:nspec_amode(8),8) = (/ 'dst_c8  ', 'so4_c8  ', 'nh4_c8  ' /)
       xname_spectype(:nspec_amode(8),8)  = (/ 'dust      ', 'sulfate   ', 'ammonium  ' /)
     ! mode 9 (coarse dust) species
       xname_massptr(:nspec_amode(9),9)   = (/ 'dst_a9  ', 'so4_a9  ', 'nh4_a9  ' /)
       xname_massptrcw(:nspec_amode(9),9) = (/ 'dst_c9  ', 'so4_c9  ', 'nh4_c9  ' /)
       xname_spectype(:nspec_amode(9),9)  = (/ 'dust      ', 'sulfate   ', 'ammonium  ' /)
#endif  
...
9213   format( 4x, a, 5(1pe14.5) )
     ...
     ! added by kzm
       modeptr_coardust1 = init_val
       modeptr_coardust2 = init_val
       modeptr_coardust3 = init_val
     !---------------------------------  
       do m = 1, ntot_amode
          if (modename_amode(m) .eq. 'accum') then
             ...
             modeptr_coardust = m
        ! added by kzm
          else if (modename_amode(m) .eq. 'coarse_dust1') then
             modeptr_coardust1 = m
          else if (modename_amode(m) .eq. 'coarse_dust2') then
             modeptr_coardust2 = m
          else if (modename_amode(m) .eq. 'coarse_dust3') then
             modeptr_coardust3 = m
    !------------
          end if
       end do
...
! added by kzm
       write(iulog,*) 'modeptr_coardust1 =', modeptr_coardust1
       write(iulog,*) 'modeptr_coardust2 =', modeptr_coardust2
       write(iulog,*) 'modeptr_coardust3 =', modeptr_coardust3
!------
       dumname = 'none'
       write(iulog,9240)
